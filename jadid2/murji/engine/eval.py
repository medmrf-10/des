...
def main():
    quran = L.load_quran()
    books = ['ibn_kathir', 'saadi', 'jalalayn', 'tabari']
    ...
    lines = []
    w = lines.append
    w('# تقييم مُرجِع — 30 آية × 4 تفاسير\n')
    ...
    w('- **الجلالين — اقتباس مجزّأ ومجموعات فارغة**: الاقتباسات شظايا '
      'قصيرة ﴿…﴾ تتخللها تراجم «أي…»، وترتيبها داخل المجموعة يشرد عن '
      'رتب المصحف أحياناً فيسقط المرشح عن السلسلة ← 3612 آية بلا موضع '
      'مثبت (conf=low)، و226 آية مجموعتها في المصدر فارغة أصلاً (`x=""`) '
      'فيرجع لها مقطع الكتلة الجارة.')
    w('- **الطبري — تثبيت إعادة الاقتباس**: تنسيق «القول في تأويل قوله '
      'تعالى: ﴿نطاق آيات﴾» يعطي علامة قوية، لكن المقدمة والتعليلات '
      'تعيد ذكر لفظ الآية قبل مقطعها الرسمي (مثلاً 1:1–1:3 ثُبّتت على '
      'إعادة اقتباس بعد مقطع الفاتحة). ونطاق الآيات المشترك '
      'يجعل آيات المجموعة الواحدة تتشارك علامة الاستشهاد ← 1755 نطاقاً '
      'منهاراً (cs==ce) وسّعها بنّاء الذرات إلى كتلتها المشتركة.')
    w('')
    w('## إعادة الإنتاج\n')
    w('```')
